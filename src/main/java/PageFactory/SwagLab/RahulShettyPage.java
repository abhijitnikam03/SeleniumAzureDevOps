...
public class RahulShettyPage {
	...
	@FindBy(xpath = "//a[@href='https://www.udemy.com/user/testing-minds/' and @class='main-btn' and contains(text(),'Access')]")
	...
	public void country_name(String cname) throws InterruptedException {
		...
		for(int i=0;i<cnt;i++) {
			...
			if(cntname.toLowerCase().equals(cname)) {
				System.out.println("country name is "+cntname);
	...
	public void checkbox(String chkbx) {
		WebElement chbx=driver.findElement(By.xpath("//input[@type='checkbox' and @value='"+chkbx+"']"));
		wait.until(ExpectedConditions.visibilityOf(chbx));
		chbx.click();
	}
	...
	public void tabledata(String course) {
		...
		String courseprc=driver.findElement(By.xpath("//table//tbody//tr//td[contains(text(),'"+course+"')]//following-sibling::td")).getText();
		System.out.println("course price is"+courseprc);
	}
	...
	public void mouseover() {
		...
		act.moveToElement(mousehover).build();
	}
